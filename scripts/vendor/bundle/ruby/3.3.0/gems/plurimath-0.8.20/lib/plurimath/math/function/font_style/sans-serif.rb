      ...
      class FontStyle
        class SansSerif < FontStyle
          ...
          def to_asciimath(options:)
            "mathsf(#{parameter_one&.to_asciimath(options: options)})"
          end

          def to_latex(options:)
            "\\mathsf{#{parameter_one&.to_latex(options: options)}}"
          end
          ...
          def to_omml_without_math_tag(display_style, options:)
            font_styles(display_style, sty: "p", scr: "sans-serif", options: options)
          end
        end
      end
    end
